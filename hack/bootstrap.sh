...
function _bootstrap_project {
  ...
  sed -i'' -e "s|gardener-extension-traefik|${EXTENSION_REPO}|g" "${_dst_path}/go.mod"
  sed -i'' -e "s|gardener-extension-traefik|${EXTENSION_REPO}|g" "${_dst_path}/internal/tools/go.mod"
  ...
  find "${_dst_path}" \
       ...
       -exec sed -i'' \
         ...
         -e "s|Value:\s*\"gardener-extension-traefik\"|Value: \"${EXTENSION_NAME}\"|g" \
         -e "s|Name:\s*\"gardener-extension-traefik\"|Name: \"${EXTENSION_NAME}\"|g" \
         -e "s|Namespace = \"gardener_extension_example\"|Namespace = \"${_extension_name_lc}\"|g" \
         -e "s|Name = \"example\"|Name = \"${ACTUATOR_NAME}\"|g" \
         -e "s|Value:\s*\"gardener-extension-traefik-leader-election\"|Value: \"${EXTENSION_NAME}-leader-election\"|g" \
         -e "s|ExtensionType = \"example\"|ExtensionType = \"${EXTENSION_TYPE}\"|g" \
         -e "s|FinalizerSuffix = \"gardener-extension-traefik\"|FinalizerSuffix = \"${EXTENSION_NAME}\"|g" \
         -e "s|gardener-extension-traefik|${EXTENSION_REPO}|g" {} \;

  _msg_info "Fixing README ..."
  sed -i'' \
      -e "s|github.com/dnaeon/gardener-extension-traefik|${EXTENSION_REPO}|g" \
  ...
  find "${_dst_path}" -type f \
       ...
       -exec \
         sed -i'' \
           -e "s|gardener-extension-traefik|${EXTENSION_NAME}|g" \
           ...
           -e "s|dnaeon/gardener-extension-traefik|${EXTENSION_NAME}|g" \
           {} \;
