...
// WithWebhookPort is an [Option], which configures the [manager.Manager] to
// use the given port for the webhook server.
func WithWebhookPort(port int) Option {
	opt := func(m *mgr) error {
		if m.webhookServer == nil {
			m.webhookServer = webhook.NewServer(webhook.Options{
				Port: port,
			})
		}
...
// WithWebhookCertDir is an [Option], which configures the [manager.Manager] to
// use the given directory for webhook certificates.
func WithWebhookCertDir(certDir string) Option {
	opt := func(m *mgr) error {
		if m.webhookServer == nil {
			m.webhookServer = webhook.NewServer(webhook.Options{
				CertDir: certDir,
